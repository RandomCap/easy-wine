...
<!--[if (gte IE 9)|!(IE)]><!--><html lang="en"> <!--<![endif]-->
    <head>
		...
		function backfillArea( id , name ){
			pup.dialog.top.noteFunction.backfillArea( id , name );
				...
				<div class="eight columns bottom-3">
					 <table class="style">
					 		<tbody>
					 			...
					 			<c:forEach var="area" items="${areas}" varStatus="index">
					 				<tr ondblclick="backfillArea( ${area.id}, '${area.name}');" style="cursor: pointer;">
					 					<td>${index.index+1}</td>
										<td> ${area.name} </td>
						            </tr>
					 			</c:forEach>
